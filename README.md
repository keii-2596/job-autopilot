...
仓库随附一份去重后的公开职位快照。控制台默认筛选“计算机 / 开发 / Agent”方向，也可以按前端、后端、全栈、算法、数据、安全、测试、运维、移动端、嵌入式等方向筛选。
...
- 独立控制台：在普通浏览器中直接和 Codex 对话、启动或恢复长期任务，并维护申请记录、个人资料和确认项。
...
独立控制台通过本机 Codex App Server 创建或恢复一个长期任务。你可以在网页里直接发送自然语言指令；任务运行中发送的内容会追加到当前轮次，回复和运行状态会持续回传。需要补充资料、处理执行环境权限或确认最终提交时，网页会显示独立确认项。MCP 嵌入式控制台仍会把请求交给当前 Codex 对话。普通 Codex 对话中的插件任务也会把安全的阶段摘要写入同一活动流；手机号、邮箱、验证码、简历正文和表单答案不会进入这份前端日志。

网页消息自动携带 `$job-autopilot` 和对应技能附件。在“个人资料 → 网页会话所属项目”点击“选择已有项目”，保存后，新建与恢复会话时都会写入该项目的正式归属并使用对应目录；未设置时使用本地状态目录下的 `workspace`。项目设置只保存在本机。

同一会话同时只能由一个运行端接管。如果桌面提示“已在另一个应用中打开”，在网页点击“移交桌面”，再到桌面点击“重试”；网页会暂停当前轮次并释放连接。正常结束一轮后也会自动释放，下次发送时恢复同一个会话。

自动推进白名单每行一条，支持精确域名、`*`（所有网站）、`*.example.com`（仅子域名）、`?`（单个字符）和 `re:^jobs\d+\.example\.com$`（正则完整匹配域名）。“检查网站”可验证已保存的规则。规则只控制自动推进范围，最终提交仍需确认。

## 数据与隐私

控制台左侧的“插件更新”可检查 GitHub 新提交、安装并重启。自动更新默认关闭，
开启后仅在后台运行且任务空闲时安装。合并、备份和失败恢复的边界见 [更新说明](docs/plugin-updates.md)。

职位库也可独立[从 GitHub 增量同步](docs/job-library-sync.md)：新增岗位、更新公开字段，
保留本地备注和投递记录；自动同步默认关闭，开启后后台每 6 小时检查，无需重启。
